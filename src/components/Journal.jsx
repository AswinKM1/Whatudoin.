...
function TodayView({ user }) {
    ...
    const handleExportToday = () => {
        const textContent = todaysEntries.map(entry => {
            const time = entry.createdAt?.toDate ? format(entry.createdAt.toDate(), 'h:mm a') : 'Unknown Time';
            return `[${time}] ${entry.text}`;
        }).join('\n\n');

        const header = `Journal Entries for ${format(new Date(), 'yyyy-MM-dd')}\n${'-'.repeat(40)}\n\n`;
        ...
        link.href = url;
        link.download = `journal_${format(new Date(), 'yyyy-MM-dd')}.txt`;
        link.click();
    };
...
function HistoryView({ user }) {
    ...
    const handleExportHistory = () => {
        ...
        const header = `Journal Entries for ${format(selectedDate, 'yyyy-MM-dd')}\n${'-'.repeat(40)}\n\n`;
        const fullContent = header + textContent;
        ...
        const link = document.createElement("a");
        link.href = url;
        link.download = `journal_${format(selectedDate, 'yyyy-MM-dd')}.txt`;
        link.click();
    };
